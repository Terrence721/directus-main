...
| 🚧 Source tree: `packages/types` (in progress)   | 35 of ~54 planned files landed — all 16 top-level modules through `geometry.ts`, `graphql.ts`, `server.ts`, `export.ts`, `import.ts`, `items.ts`, all 13 `extensions/*` submodules, and both `index.ts` files. `src/index.ts` already exports from 19 modules that don't exist yet: `mail.ts`, `misc.ts`, `notifications.ts`, `payload.ts`, `permissions.ts`, `policies.ts`, `presets.ts`, `query.ts`, `relations.ts`, `schema.ts`, `services.ts`, `settings.ts`, `shares.ts`, `snapshot.ts`, `storage.ts`, `users.ts`, `versions.ts`, `vue.ts`, `websockets/index.ts` — the package doesn't type-check as a whole until those land, same "manifests first" tolerance as everything else mid-migration                      |

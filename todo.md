...
| 🚧 Source tree: `packages/types` (in progress)   | 40 of ~54 planned files landed — all 16 top-level modules through `geometry.ts`, `graphql.ts`, `server.ts`, `export.ts`, `import.ts`, `items.ts`, `mail.ts`, `misc.ts`, `notifications.ts`, `payload.ts`, `permissions.ts`, all 13 `extensions/*` submodules, and both `index.ts` files. `src/index.ts` already exports from 14 modules that don't exist yet: `policies.ts`, `presets.ts`, `query.ts`, `relations.ts`, `schema.ts`, `services.ts`, `settings.ts`, `shares.ts`, `snapshot.ts`, `storage.ts`, `users.ts`, `versions.ts`, `vue.ts`, `websockets/index.ts` — the package doesn't type-check as a whole until those land, same "manifests first" tolerance as everything else mid-migration. **`permissions.ts` additionally introduces this package's first cross-package runtime dependency**: `import { PERMISSION_ACTIONS } from '@directus/constants'` won't resolve until `packages/constants` (currently manifest-only, like the other 31 remaining packages below) gets real source — a gap that outlives `packages/types` finishing its own 54 files, distinct from the within-package forward-references above |

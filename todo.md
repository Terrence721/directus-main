...
| `packages/types` package                      | Full source (56 files, added one commit at a time and diffed byte-for-byte against upstream throughout) — revised up from a ~54 estimate once `websockets/` turned out to be a 4-file submodule (`base.ts`, `collab.ts`, `index.ts`, `type.ts`), the same shape as the 13 `extensions/*` submodules. **Doesn't fully type-check yet**: three cross-package dependencies are still manifest-only — `permissions.ts` → `@directus/constants`, `relations.ts` → `@directus/schema`, `settings.ts` → `@directus/ai`. Also diverges from upstream by one line in `tsconfig.json`: an explicit `"rootDir": "./src"`, since neither `@directus/tsconfig`'s `base` nor `node22` config sets one — real `tsc` 5.9.3 builds identically either way, but newer/preview TS tooling flags the implicit inference as an error. |
...
**Still to do:** full source trees for 38 of 40 workspace packages (everything except `directus/` and `types/`, both now
fully sourced) and a handful of tracked follow-ups — see the "Still to do" section below.
...
- All 17 workflows migrated off pnpm (`pnpm exec` → `yarn exec`, `pnpm run` → `yarn run`, `pnpm --recursive publish` →
...
| ⚠️ CI secrets not yet configured                 | `Check` now installs cleanly (see the `isolated-vm` fix above), but workflows depending on secrets (`CLAUDE_CODE_OAUTH_TOKEN`, `RELEASE_PAT`, `DOCKERHUB_USERNAME`/`PASSWORD`, `SLACK_WEBHOOK_CMS_FREEZE`) aren't configured on `Terrence721/directus-main` yet. `Sync Readme to Docker Hub`'s own step is now gated `if: false` (re-enable note in the workflow comment) since it kept showing red on every `readme.md` push for a gap already known, not a new failure — the other secret-dependent workflows trigger less often so haven't needed the same treatment yet                                                                                                                                                 |

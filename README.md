...
Three packages done: `packages/stores` (four Pinia stores, 20 tests, 100% coverage), `packages/constants` (RBAC/auth vocabulary, 2 tests), and `packages/errors` (a real `DirectusError` class hierarchy — `ForbiddenError`, `InvalidCredentialsError`, 10 tests). `app/` is a real, deployed Vue app — routed (`vue-router`), styled, with a working login/logout flow against a simulated `authClient`, and a session that now persists across a hard refresh. `api/` is in progress: a real Express server with a tested `POST /auth/login`, not yet wired into `app/`. Root scaffolding (Yarn Berry workspace, CI, lint/format tooling) is complete. See `todo.md` for the exact commit-by-commit breakdown.

Next: growing `api/` into a real backend and wiring it into `app/`, plus more of the real 31-package target.

...
**[terrence721.github.io/directus-main/app](https://terrence721.github.io/directus-main/app/)** — a real, deployed Vue app: routed login/home pages, a working (simulated) auth flow whose session now survives a hard refresh, and a logout button. Try `demo@directus-main.dev` / `demo1234`, or anything else to see the real error path.
...
- **[Testing Strategy](https://terrence721.github.io/directus-main/diagrams/testing-strategy.html)** — the workspace test-layer stack, and what 100% coverage still let through
...
Three packages done: `packages/stores` (four Pinia stores, 20 tests, 100% coverage), `packages/constants` (RBAC/auth vocabulary, 2 tests), and `packages/errors` (a real `DirectusError` class hierarchy — `ForbiddenError`, `InvalidCredentialsError`, 10 tests). Two more in progress: `packages/system-data` (the real `types.ts` type surface, done, 30 tests) and `packages/utils` (`toBoolean`/`toArray` done so far, 27 tests, 61 real functions total). `app/` is a real, deployed Vue app — routed (`vue-router`), styled, with a working login/logout flow against a real `authClient` that genuinely calls `api/` in local dev (the deployed GitHub Pages build still uses a simulated fallback, since `api/` isn't hosted anywhere public), a session that now persists across a hard refresh, and the beginnings of a real module-registration system (`defineModule`, one registered module). `api/` itself is in progress: a real Express server with a tested `POST /auth/login`. Root scaffolding (Yarn Berry workspace, CI, lint/format tooling) is complete. See `todo.md` for the exact commit-by-commit breakdown.

Next: growing `api/` further, continuing `packages/utils`, and wiring the new module system into `app/`'s router.

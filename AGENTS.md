...
Directus is a real-time API and App dashboard for managing SQL database content. This is a Yarn (Berry) monorepo containing:
...
yarn workspace @directus/api build

# Run development servers (API on :8055, App on :8080)
cd api && yarn dev    # API with hot reload
cd app && yarn dev    # App with Vite HMR
...
yarn workspace @directus/api test   # Test specific package
cd api && yarn test:watch           # Watch mode in package
yarn test:coverage                  # Coverage report

# Blackbox/E2E tests (requires building first)
yarn test:blackbox
TEST_DB=postgres yarn test:blackbox  # Against specific database
```
...
- Pin external dependency versions directly in each package's `package.json` (Yarn has no shared-catalog mechanism)
- When a dependency is used by multiple packages, keep the version consistent across them where practical
...
```bash
yarn lint         # ESLint - checks JavaScript/TypeScript code
yarn lint:style   # Stylelint - checks CSS/SCSS/Vue styles
yarn format       # Prettier - checks code formatting
```
